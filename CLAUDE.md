...
| The STALENESS surface — `service.WiringPosture` / `service.WiringHealth` / `WithWiringHealth` / `requireWiringAdmin` (`service/wiring_posture.go`), the `wiringPoll.alarm` / `wiringPoll.refreshed` seam or `wiringRefreshAlarm` (`internal/cli/wiring_stale.go`), or the `WiringPosture` rpc | `skills/api-surface.md` ("The wiring posture read" — the ONE full account — **and** the facade surface list), `docs/src/library/service-facade.md` ("The wiring posture — a system-tier read" + the `WithWiringHealth` row in the options table), `docs/src/surfaces/rpc-reference.md` ("Deployment posture and health"), the `APERTURE_WIRING_REFRESH_FAILED` fixups in `errors/codes.go` — **and `make proto`** when the message moves, **and `make docs-gen`** | reviewed; no registry gate — behaviour by `service/wiring_posture_test.go`, `internal/cli/wiring_stale_test.go` and `internal/server/wiring_posture_test.go`. **`Capabilities` DELIBERATELY DOES NOT ABSORB THIS, and the reasoning lives in three doc comments** (`service/wiring_posture.go`'s file header, `service.Capabilities`, and the rpc's own proto comment) because the pull to add one more boolean there is strong: `Capabilities` is OPEN precisely because it carries booleans of IMMUTABLE BOOT-TIME configuration and cannot fail, where staleness is mutable runtime FAULT state whose useful half is a DURATION, the admin shell caches `Capabilities` once on page load, and "this instance has been enforcing configuration its operator already replaced, for four hours" tells an anonymous caller that the enforced policy is not the intended one and how long the window has been open. A bare boolean left open with the duration behind auth is THEATRE — the boolean carries the disclosure. `TestCapabilitiesDidNotAbsorbTheStaleness` pins it. **The check ORDER is the contract**: the gate runs before the recorder is consulted, so a refusal is byte-identical for a healthy, an unpolling and a four-hours-stale instance, or the refusal becomes a probe for "is this instance degraded?". **An unwired recorder is an ANSWER and not a refusal** — the deliberate difference from `ListAttributes` — or the read stops being usable as a fleet-wide probe. **The alarm PASSES THE UNDERLYING CODE THROUGH** (`wiringRefreshAlarm`'s guard) and `APERTURE_WIRING_REFRESH_FAILED` is the classification of last resort: burying `APERTURE_WIRING_CONNECTION_UNROUTED` costs the operator the fixups that name the environment variable to export, so the tests assert chain DEPTH. **RECOVERY CLEARING IT IS HALF THE STORY** — any successful refresh, including one that saw no change (almost every tick of almost every deployment), resets the window, the count and the code — and a latching alarm passes every test that only checks it fired. **ONE reporter, not one per failure mode**: an unreachable store, an undigestible set, wiring this host cannot build and a failed rebuild are four ways of arriving at one fact, and the rebuild story attaches to `p.alarm(err)` / `p.refreshed()` without restructuring |
