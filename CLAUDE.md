...
| Where a booting instance's runtime WIRING comes from (`internal/cli/store.go`'s `readSharedWiring` and `seedDocument`'s durable branch, `internal/cli/decision.go`'s choice between the local and the projected document, or `internal/cli/wiring_boot.go`'s `wiringDocument` / `connectionRoutes` / `connectionDSNEnvVar`) — **or where a RUNNING instance's wiring comes from** (`internal/cli/wiring_swap.go`'s `liveWiring` / `wiringVersion` / `wiringRebuild` / `borrowBootPools`, `internal/cli/decision.go`'s `buildWiredStack`, or `serve`'s `serveFacadeOptions` and the `liveWiring` it mounts as its handler) | `docs/src/cli/global-options.md` ("It means the same thing for *wiring*", which states the three connection ROUTES and the `APERTURE_CONNECTION_<NAME>_DSN` spelling), `docs/src/cli/serve.md` ("Where its wiring comes from" **and** "What a swap replaces, and what it does not", plus the paragraph pointing at it in `docs/src/cli/global-options.md`), and the row above — an omitted `--seed` now means the same thing for wiring as it does for model state, so the two move together | reviewed; no registry gate — behaviour by `internal/cli/wiring_boot_test.go`. **Empty wiring tables must keep meaning "use the local file"**: an empty `model.WiringSet` is an ANSWER (`IsEmpty`), not a failure, and every deployment that has never run `aperture wiring push` is in that branch — turning it into a refusal, or adding a flag to select it, breaks every existing single-instance deployment at once. **The projection goes back onto a `seed.Document` on purpose**: a parallel builder over `model.Wiring*` would restate the statement contract, the kind vocabulary, the ttl parsing, the reference-target check, the field-type words and the slot set, and a divergence between the two does not error — it authorizes differently on two instances that were supposed to be identical. **The shared row carries the connection NAME and never a route**; `seed.WithConnectionOpener` stays the only seam a Go host needs, and the derived environment variable is the CLI's route of last resort, not a second mechanism. For the RUNNING half, behaviour is `internal/cli/wiring_swap_test.go`: **a version is installed whole or not at all, and a superseded version is never mutated and never `Close`d** — its pools are BORROWED from the boot, so its `Close` would be the serving instance's. Readers do one `atomic.Pointer.Load` and the mutex is writers-only, which is what keeps a decision from ever blocking on a wiring read. **The facade extras must be re-composed from the SAME function as the boot** (`serveFacadeOptions`): compose them anywhere else and the authority gate, delegation and impersonation go on deciding through the superseded engine while `Check` answers through the new one — two engines in one process, which no verdict, trace or note reports. The same reasoning is why the whole facade is rebuilt per version rather than only the engine: `service.WithProviders` / `WithAttributes` take CONCRETE registries, so a change that stopped rebuilding it would silently leave `ObjectIdentifiers`, `ObjectMetadataBatch` and `Simulate`'s overlay fetch reading the BOOT registries |
