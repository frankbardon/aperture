...
| The seed `attribute_providers:` schema (a `seed.AttributeProvider` YAML key, the `kind:` set in `attributeKinds()`, or the per-slot `ttl:` / `max_size:` handling) or the layering reporters (`Document.AttributeSlotSources` / `AttributeSourceInline` / `AttributeCollisions`) | the field's doc comment, `skills/attribute-providers.md` ("From a seed document", "Precedence: two layers, and the shared layer wins"), `docs/src/concepts/seed.md` ("External attribute sources", including "Precedence: two layers, and the shared layer wins"), and the `attributes slots` output in `internal/cli/attributes.go` — the CLI reads the rule from `AttributeSlotSources` and must never re-derive it | reviewed; no registry gate — behaviour by `seed/attribute_provider_test.go`, `seed/attribute_provider_csv_test.go`, `seed/attribute_provider_sql_test.go`. **The `attribute_providers:` entry is the slot's SHARED layer and the inline `attributes:` block its LOCAL one; a fetch reads their merge and the shared layer wins every key both serve** — nothing is dropped — **and the layering is reported, never silent**: the report names which layer answers a contested key. `AttributeSlotSources` therefore names the WINNER for a slot both sections fill, and a surface that needs both layers asks `provider.AttributeRegistry.Layers`. The row below is the full account |
| The attribute-layering PRECEDENCE — `provider.AttributeLayer` / `AttributeLayers()`, which layer `AttributeRegistry.Register` and `RegisterLocal` fill, `mergeAttributeBags` / `mergeAttributeRecords` stamping the SHARED layer last, or which layer `CacheConfigFor` calls governing | ALL of: the `provider/attribute_layer.go` file doc (the one full account) and the two registration methods' doc comments; `skills/attribute-providers.md` ("From Go", "From a seed document", "Precedence: two layers, and the shared layer wins", "The floor bags, and why the floor wins", "The leniency contract", "Staleness is a security window, not a tuning knob", "`Enumerate` never writes the slot's cache", "The operator surface" and the `APERTURE_ATTRIBUTE_PROVIDER_INVALID` row) plus its frontmatter `description`; `skills/rules-engine.md` ("Context variables", both the `principal` and the `account` root); `docs/src/concepts/seed.md` ("Inline subject attributes", "External attribute sources", "Precedence: two layers, and the shared layer wins"); `docs/src/concepts/providers.md` ("The registry, the two layers, and the revocation window", "Leniency", "The listing does not write the decision path's cache", "Where the bags come from"); `docs/src/cli/attributes.md` ("`slots`" and "`invalidate`"); the `seed/` doc comments that state it (`Document.AttributeCollisions`, `AttributeSlotSources`, `buildAttributeRegistry`, `seed.AttributeProvider.DeclaredKeys`, `seed/attribute.go`'s file doc); `internal/cli/decision.go`'s `attributeCollisions` field comment AND the boot warning it prints; `internal/cli/attributes.go`'s `slots` description — **and `make docs-gen`**; `internal/cli/wiring_boot.go`'s `layerAttributeProviders`; and the `APERTURE_ATTRIBUTE_PROVIDER_INVALID` fixups in `errors/codes.go` — **`make docs-gen`** again, since the generated code table restates them | reviewed; no registry gate — behaviour by `provider/attribute_layer_test.go`, `provider/attribute_test.go`, `seed/attribute_provider_test.go`, `seed/attribute_provider_csv_test.go`, `rules/principal_floor_test.go` and `rules/account_floor_test.go` (both pass UNCHANGED, which is the point). **This reversed a written decision**: the external source used to win a slot ENTIRELY and the inline bags were discarded, which made the two mutually exclusive, so an instance could not add a field the shared directory does not carry without abandoning the directory. Do not restate the discard anywhere. **SHARED WINS, unconditionally and independently of registration order**, because a rule is written against a DEPLOYMENT: if a local bag could override a shared key, a file on one machine would change what `principal.clearance >= 3` compares against on that machine only — the same grant, a different verdict, with nothing in a verdict, a trace or a note to say which layer answered. **Merging as PEERS is the other regression** — whichever registered last would silently redefine every key — so the relaxation is a second LAYER with a fixed winner, never a second candidate. **The FLOOR is unchanged and still stamps over both** (`rules.principalBag` / `accountBag`): the three tiers compose in one direction, floor over shared over local, and a doc that folds the floor into the layering has lost the guarantee that stops `principal.id == object.owner` comparing something else. **TWO CACHES, NOT ONE**, because a layer's `ttl:` is its own revocation window: pooling them would either lengthen the window a revoked shared attribute keeps authorizing for or ignore a declaration an operator made, so every `Invalidate*` clears every layer and documenting the ttl as one number per slot is the regression |
...
| The `aperture wiring` COMMAND TREE (`internal/cli/wiring.go` / `internal/cli/wiring_diff.go`: a subcommand, a flag, a command description, or `wiringDriftExitCode`) | `docs/src/cli/wiring.md` (the narrative page — the ONE operator-facing account — plus its row in `docs/src/cli/overview.md` and its `SUMMARY.md` entry), "The five tables, and the command tree" and "`wiring diff` exits 2, and carries no code" in `skills/shared-wiring.md`, the three-question table in `docs/src/operations/two-instance-topology.md` ("Verifying the pair agrees"), and the `APERTURE_WIRING_*` fixups in `errors/codes.go` — **plus `make docs-gen`**, since `docs/src/reference/cli.md` is generated from the flags and descriptions | reviewed; no registry gate — behaviour by `internal/cli/wiring_test.go`, `wiring_refuse_test.go`, `wiring_pull_test.go`, `wiring_diff_test.go`, `wiring_declared_keys_test.go`, and the `push → pull → push` fixed point plus the two-backend parity half in the gated `TestPostgresLive ./internal/cli/` run. **CLI-ONLY is the scope decision, not an unfinished edge**: a push changes what every decision in every instance sees at once, so it requires a human at a shell holding both the store credential and the document — there is no RPC method, no MCP tool and no admin-UI panel, and MCP stays read + decide + simulate forever. **NONE of the four takes an actor and `--store` has no default on any of them**: the store credential is the authority (as for `aperture import`), the three reads restate wiring that credential already grants full write access to, and an in-memory store has no shared wiring either to write or to read. **`diff` exits 2 on drift and carries NO `APERTURE_*` code** — `aperture check`'s clean-deny precedent; 2 and not 1 because every coded refusal exits 1, so merging them makes a gate that fails closed on drift also fail closed on a typo'd DSN and report it AS drift. Reclassifying a `kind: csv` local-only row as drift is the other regression: no push can make the deployment match it, so the gate could never go green |
| The FOUR-SHARED / TWO-LOCAL split itself — which seed sections `aperture wiring push` shares, or which read-back emits which (`seed.Export` the model and no wiring, `seed.MarshalWiring` the four shared sections and no model, nothing the two local ones) | EVERY statement of it moves together, because the old "the seed FILE is the source of truth for all six" contract was written in a dozen places: `seed/seed.go`'s `Document` doc comment (the primary statement) and its `Connections` / `Attributes` / `AttributeProviders` field comments, then the per-section comments in `seed/provider.go` (struct + `References`), `seed/connection.go`, `seed/fields.go`, `seed/attribute_provider.go`, `seed/object.go` and `seed/attribute.go`, plus `seed/export.go`'s `Export`; "The four/two split" in `skills/shared-wiring.md`; `skills/sql-provider.md`, `skills/metadata-values.md`, `skills/object-references.md` and `skills/attribute-providers.md` ("From a seed document"); `docs/src/concepts/seed.md` (the section list at the top, "The file is not the only home for wiring", and "What is *not* in the file"), `docs/src/concepts/providers.md`, `docs/src/cli/wiring.md`, `docs/src/cli/global-options.md` and `docs/src/operations/two-instance-topology.md` | reviewed; no registry gate — behaviour by `internal/cli/wiring_project.go`'s own tests plus `internal/cli/wiring_identical_test.go`. **"`Apply` writes no wiring" and "an export reproduces no wiring" are both STILL TRUE and neither says where wiring lives** — conflating the three read-backs is exactly how the repository came to contradict itself, and a doc that says the file is the only home sends an operator to edit a file the database is overriding. **The split is a pointer-to-data versus the-data-itself line, not a staging order**: `objects:` and `attributes:` carry the metadata and the bag, and Aperture's own database is never the source of truth for a host's domain data — the same Non-Goal that keeps the provider cache out of an export — so there is nowhere for them to go and no story that moves them. **Keeping `Export` wiring-free is a security property**: it is reachable over Twirp with an admin-tier token, where the wiring read-back is CLI-only behind the store credential |
